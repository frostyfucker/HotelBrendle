...
import { Moon, Sun, Palette } from './components/Icons';
...
const ROLES_CONFIG: Record<UserRole, { views: View[], default: View }> = {
  admin: {
    views: ['dashboard', 'hotel-pulse', 'maintenance-repair', 'renovation-advisor', 'budget-tracker', 'inventory', 'session-summarizer'],
    default: 'dashboard'
  },
  staff: {
    views: ['dashboard', 'hotel-pulse', 'maintenance-repair', 'time-clock', 'session-summarizer'],
    default: 'dashboard'
  },
  guest: {
    views: ['guest-services', 'local-guide'],
  ...
  return (
    <>
      <div className="flex h-screen bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans">
        <Sidebar activeView={activeView} setActiveView={handleSetView} onOpenAbout={() => setIsAboutModalOpen(true)} />
        <div className="flex-1 flex flex-col overflow-hidden">
          <header className="bg-white dark:bg-slate-800/50 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700 p-4 shadow-sm z-20 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-brand-primary-600 dark:text-brand-primary-400">
              Hotel Brendle Orchestrator <span className="text-slate-400 dark:text-slate-500 font-normal text-xl">// Renovation Project</span>
            </h1>
