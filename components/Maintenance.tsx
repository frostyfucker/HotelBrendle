...
const Maintenance: React.FC = () => {
    ...
    const handleAnalyzeVideo = async () => {
        ...
        try {
            ...
            const storedInventoryJSON = localStorage.getItem(userInventoryKey);
            const inventory: InventoryItem[] = storedInventoryJSON ? JSON.parse(storedInventoryJSON) : [];
    ...
    const handleSubmitWorkOrder = (e: React.FormEvent) => {
        ...
        try {
            const hotelDataKey = `${user.id}-hotelBrendleData`;
            const expensesKey = `${user.id}-renovationExpenses`;
            
            const hotelDataJSON = localStorage.getItem(hotelDataKey);
            if (!hotelDataJSON) throw new Error("Hotel data not found in storage.");
            
            const hotelData: Building[] = JSON.parse(hotelDataJSON);
            let roomFound = false;
            
            const updatedHotelData = hotelData.map(building => ({
                ...building,
                floors: building.floors.map(floor => ({
                    ...floor,
                    rooms: floor.rooms.map(room => {
                        if (room.number.toLowerCase() === location.toLowerCase()) {
                            roomFound = true;
                            ...
                            room.workOrders = [newWorkOrder, ...(room.workOrders || [])];
                        }
                        return room;
                    })
                }))
            ...
            localStorage.setItem(hotelDataKey, JSON.stringify(updatedHotelData));
            ...
            const existingExpensesJSON = localStorage.getItem(expensesKey);
            ...
            window.dispatchEvent(new StorageEvent('storage', { key: hotelDataKey }));
            window.dispatchEvent(new StorageEvent('storage', { key: expensesKey }));
            ...
            console.error("Failed to update data in localStorage", storageError);
            setError(storageError instanceof Error ? storageError.message : "A critical storage error occurred.");
            return;
        }
    };
